    ...
    def get(self, request, pk=None):
        ...
        if pk:
            try:
                ...
                # Suggested products: active products sharing the same generic name,
                ...
                if product.generic_name_id:
                    ...
                    suggested_products = ProductSerializer(suggested_qs, many=True).data
...
class ProductSearchView(APIView):
    ...
    def get(self, request):
        ...
        try:
            ...
            Q(product_name__istartswith=query)).filter(is_active=True).distinct()
...
class AddProductToBatch(APIView):
    def post(self, request):
        ...
        old_stock_quantity = Product.objects.get(pk=product_id).stock_quantity
        ...
        if check_batch:
            ...
            same_product_check = TempProduct.objects.filter(is_confirmed=False, user_id=user, product_id=product_id).first()
            if same_product_check:
                same_product_check.new_stock_quantity += new_stock_quantity
                same_product_check.new_cost_price = new_cost_price
                same_product_check.new_selling_price = new_selling_price
                same_product_check.save()
                return Response(
                    {
                        "message": "Product staged successfully",
                        "batch_id": str(same_product_check.batch_id),
                        "temp_id": same_product_check.id,
                    },
            ...
            batch_id = f"BATCH-{date_part}-{random_part}"
...
class BatchSummary(APIView):
    def get(self, request, batch_id):
        updates = TempProduct.objects.filter(batch_id=batch_id, is_confirmed=False)
        ...
        data = [
            {
                ...
                "old_stock": u.old_stock_quantity,
...
class ProductDelete(APIView):
    def delete(self, request, product_id):
        try:
            product = TempProduct.objects.get(id=product_id, is_confirmed=False)
            product.delete()
            return Response(
            ...
            return Response(
                {"error": "Product not found or already confirmed"},
                status=status.HTTP_200_OK
            )

class GetBatchSummary(APIView):
    def get(self, request, batch_id):
        updates = TempProduct.objects.filter(batch_id=batch_id, is_confirmed=True)
        if not updates:
            return Response({"message": "No batch found"}, status=status.HTTP_404_NOT_FOUND)

        if not updates.exists():
            return Response({"message": "No products in this batch"}, status=status.HTTP_404_NOT_FOUND)

        total_value = sum([u.total_amount for u in updates])
        created_on = str(updates.first().created_on)
        data = [
            {
                "id": u.id,
                "product": u.product_id.product_name,
                "stock": u.new_stock_quantity,
                "current_stock": u.new_stock_quantity,
                "cost_price": str(u.new_cost_price),
                "mrp": str(u.mrp),
                "selling_price": str(u.new_selling_price),
                "total": str(u.total_amount),
...
class ConfirmBatch(APIView):
    def post(self, request, batch_id):
        ...
        for update in updates:
            ...
            add_qty = Decimal(update.new_stock_quantity or 0)
            add_cost = Decimal(update.new_cost_price or 0)

            total_qty = old_qty + add_qty
            if total_qty > 0:
                weighted_cost = ((old_qty * old_cost) + (add_qty * add_cost)) / total_qty
                product.cost_price = weighted_cost.quantize(Decimal("0.01"))
            else:
                product.cost_price = add_cost
...
# class BatchSearchAPIView(APIView):
#     """
#     API to search unique batch IDs.
#     Use query parameter: ?q=<search_term>
#     """
#     def get(self, request):
#         search_query = request.GET.get("q", "").strip()  # Get search term

#         # Get distinct batch_ids
#         queryset = TempProduct.objects.values_list("batch_id", flat=True).distinct()

#         if search_query:
#             # Filter batch_ids containing the search term
#             queryset = [bid for bid in queryset if search_query.lower() in str(bid).lower()]

#         # Return as list of dicts
#         data = [{"batch_id": bid} for bid in queryset]

#         return Response(
#             {"status": "success", "data": data},
#             status=status.HTTP_200_OK,
#         )
class BatchSummaryByDate(APIView):
    ...
    def get(self, request):
        ...
        base_qs = TempProduct.objects.filter(
        ...
        if not base_qs.exists():
            return Response(
                {"message": "No batch found in this date range"},
                status=status.HTTP_404_NOT_FOUND
            )

        # Grand totals from DB
        grand_totals = base_qs.aggregate(
            grand_total_products=Count("id"),
            grand_total_value=Coalesce(Sum("total_amount"), Decimal("0.00")),
        )

        # Batch-wise totals from DB
        batch_totals = base_qs.values("batch_id").annotate(
            total_products=Count("id"),
            total_value=Coalesce(Sum("total_amount"), Decimal("0.00")),
        ).order_by("batch_id")

        # Product rows
        product_rows = base_qs.select_related("product_id").values(
            "id",
            "batch_id",
        ...
        for row in product_rows:
            batches_map[row["batch_id"]]["products"].append({
                "id": row["id"],
                "product": row["product_id__product_name"],
                "stock": row["new_stock_quantity"],
                "cost_price": str(row["new_cost_price"]),
                "mrp": str(row["mrp"]),
                "selling_price": str(row["new_selling_price"]),
        ...
        return Response({
            ...
            "grand_total_products": grand_totals["grand_total_products"],
            "grand_total_value": str(grand_totals["grand_total_value"]),
...
class BatchSearchAPIView(APIView):
    ...
    def get(self, request):
        ...
        if not start_date or not end_date:
            qs = TempProduct.objects.filter(is_confirmed=True)

            if search_query:
                qs = qs.filter(batch_id__icontains=search_query)

            # Newest batch first: order by each batch's most recent created_on.
            batch_rows = (
                qs.values("batch_id")
                .annotate(latest_created=Max("created_on"))
                .order_by("-latest_created")
            )

            data = [{"batch_id": row["batch_id"]} for row in batch_rows]
...
class LowStockProductCountAPIView(APIView):
    """
    Returns the count of products where stock_quantity < 2
    """

    def get(self, request):
        count = Product.objects.filter(
            stock_quantity__lt=2,
            is_active=True
        ).count()

        low_stock_products = Product.objects.filter(stock_quantity__lt=2)
        serializer = ProductSerializer(low_stock_products, many=True)

        return Response(
            {
                "status": "success",
                "low_stock_product_count": count,
                "data": serializer.data
            },
            status=status.HTTP_200_OK
        )
...
class NegativeStockProductAPIView(APIView):
    """
    Returns products whose stock_quantity is negative, with aggregated stats
    (total deficit units, restock cost at cost price, lost retail value at MRP).
    """

    def get(self, request):
        from decimal import Decimal

        products_qs = Product.objects.filter(stock_quantity__lt=0)
        total_count = products_qs.count()
        active_count = products_qs.filter(is_active=True).count()

        total_deficit_units = 0
        total_restock_cost = Decimal("0.00")
        total_retail_value = Decimal("0.00")
        total_selling_value = Decimal("0.00")

        for p in products_qs:
            deficit = abs(int(p.stock_quantity or 0))
            total_deficit_units += deficit
            total_restock_cost += (Decimal(str(p.cost_price or 0)) * deficit)
            total_retail_value += (Decimal(str(p.mrp or 0)) * deficit)
            total_selling_value += (Decimal(str(p.selling_price or 0)) * deficit)
        ...
        return Response(
            {
                "status": "success",
                "negative_product_count": total_count,
                "active_negative_count": active_count,
