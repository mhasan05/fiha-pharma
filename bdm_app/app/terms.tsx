...
import { getTermsConditions } from "@/api/settings";
...
export default function TermsScreen(): React.ReactElement {
  ...
  const { data, isLoading } = useQuery({ queryKey: ["terms-conditions"], queryFn: getTermsConditions });

  // Admin content stores each point on its own line, optionally prefixed with "#".
  ...
  return (
    <View className="flex-1 bg-canvas">
      <View className="flex-row items-center border-b border-border bg-surface px-4 pb-3" style={{ paddingTop: insets.top + 10 }}>
        ...
        <Text className="text-lg font-bold text-ink">{data?.title || t("terms")}</Text>
      </View>
      ...
      {isLoading ? (
        ...
        <ScrollView contentContainerStyle={{ padding: 16, paddingBottom: insets.bottom + 96 }} showsVerticalScrollIndicator={false}>
          {lines.length > 0 ? (
            // Admin-provided terms from the backend.
            ...
            // Professional default terms bundled with the app.
            <LegalView sections={TERMS} />
          )}
        </ScrollView>
      )}
