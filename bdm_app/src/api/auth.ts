...
import type { ApiEnvelope, Area, AuthUser, LoginResponse, SignupPayload } from "@/types/api";

/** POST /auth/login/ — phone (or email) + password. Returns { access_token, data: user }. */
export async function login(phone: string, password: string): Promise<LoginResponse> {
  const { data } = await api.post<LoginResponse>("/auth/login/", { phone, password });
  return data;
}

/** POST /auth/signup/ — shop-owner registration. */
export async function signup(payload: SignupPayload): Promise<{ status?: string; message?: string }> {
...
export async function uploadProfileImage(uri: string, fields?: Record<string, string>): Promise<AuthUser> {
  ...
  let data: (AuthUser & { message?: string; errors?: Record<string, string[]> }) | { data: AuthUser } = {} as AuthUser;
  try {
  ...
  return "data" in data ? (data as { data: AuthUser }).data : (data as AuthUser);
}

/** GET /auth/area/ — areas for the signup dropdown. */
export async function getAreas(): Promise<Area[]> {
  const { data } = await api.get<ApiEnvelope<Area[]>>("/auth/area/");
  return data.data ?? [];
}
